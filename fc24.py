...
def check_activation_code(code):
    ...
    url = 'http://fc24hunter.42web.io/subs/%s' %(code)

    sess = cloudscraper.create_scraper()

    res = sess.get(url, headers={'Cookie': '__test=4dafc5ed9a7c5adc82cdff8f7d1137fb'})

    if res.status_code == 200:

        ftp = FTP(host='185.27.134.11', user='if0_35418309', passwd='1KcKJPn0M7WDFu')
        ...
        try:
            ...
            ftp.storbinary(f'STOR {code}', 'iraq')
...
def activation_code_prompt():
 
    codeInput = input(f'\n{res}[{green}+{res}] {res}Enter The {green}Activation Code {res}:{blue} ')

    code_status = check_activation_code(codeInput)

    if code_status == True:

        with open('user_credentials.json', 'r+', encoding='utf8') as json_text:

            user_profile = json.loads(json_text.read())

            user_profile['activation_code'] = codeInput

            json_text.close()
        
        with open('user_credentials.json', 'r+', encoding='utf8') as json_text:

            json_text.write(json.dumps(user_profile))
...
def showPlayers(attackers, midfielders, defenders):

    attackers_players = attackers

    midfielders_players = midfielders

    defenders_players = defenders
    ...
    for defender in defenders_players:
...
def display_players(ptupl):
    ...
    print(f'\n{green}[{res}Attackers{green}]:{res}\n')

    for attacker in ptupl[0]:

        c += 1

        print(f'''{green}({res}{c}{green}) {red}> {res}{attacker}''')

    else:
        c = 0

    print(f'\n=====================================\n\n{green}[{res}Midfielders{green}]:{res}\n')

    for midfielder in ptupl[1]:

        c += 1

        print(f'{green}({res}{c}{green}) {red}> {res}{midfielder}')

    else:
        c = 0
    

    print(f'\n=====================================\n\n{green}[{res}Defenders{green}]:{res}\n')

    for defender in ptupl[2]:

        c += 1

        print(f'{green}({res}{c}{green}) {red}> {res}{defender}')
...
def defenders(conn):
    ...
    defenders = cr.execute('SELECT * FROM defenders')

    for defender in enumerate(defenders.fetchall(), start=1):

        defenders = defender[1][0]

        defender_id = defender[1][1]

        defenders_players[defenders] = defender_id
    ...
    with cloudscraper.create_scraper(debug=False) as sess:
        ...
        if login_webapp.status_code == 200:
            ...
            if twoFA.status_code == 200:
                ...
                sixDigitCode = input(f'\n{yellow}[!] {res}Enter 6 Digit Code From {green}Authenticator App:{res} ')
                ...
                while counter > 0:
                    ...
                    try:
                        ...
                        if counter == 1:
                            ...
                            print(f'[!] Stage {s} Completed !, Please Wait 30 Seconds To Start Stage {s + 1} ...\n')
                            time.sleep(30)
                            counter = 15
                            cls()
                            s = s + 1
...
if __name__ == '__main__':
    ...
    while True:
        ...
        avail_players = showPlayers(attackers_players, midfielders_players, defenders_players)
...
{yellow}[*] Note:{res}
            Press ENTER To Go Back To The Main Menu''')
                ...
                if player in attackers_players:
                    ...
                    with open('user_credentials.json', 'r', encoding='utf8') as file:

                        jsonob = json.loads(file.read())

                        if 'activation_code' in jsonob:
                            
                            pass

                        else:

                            sub = activation_code_prompt()

                            if sub == True:
                                
                                print(f'\n{res}[{green}+{res}] The Activation Code Is Now {green}Activated !!{res}\n\n')

                            else:

                                cls()

                                print(f'\n{yellow}[!]{red} The Activation Code Is Invalid !!{res}')
            ...
            back = input(f'\n\n{yellow}[!] {res}Press {green}ENTER {res}To Go Back To The Main Menu ')
            ...
            try:
                ...
                with open('user_credentials.json', 'r', encoding='utf8') as f:
                    ...
                    print(f'''\n{yellow}[!] {res}You Already Logged In With These Credentials:\n
{res}Email: {green}{data['email']}\n\n{res}Password: {green}{data['password']}\n''')
                    
                    action = input(f'''\n\n{yellow}[!] {res}Choose Action:
                        
                    {green}({res}1{green}) {red}> {res}Log Out
                    
                    {green}({res}2{green}) {red}> {res}Go Back
